...
getcluster_mixtools_DAPI_area <- function(data_frame, model, noise_cluster = 5){
...
data_frame$dapi_area_cluster <- as.numeric(data_frame$cluster_mod)


data_frame <- data_frame %>%
  mutate(dapi_area_cluster = case_when(
    dapi_area_cluster == 1 ~ "dead",
    dapi_area_cluster == 2 ~ "G01",
    dapi_area_cluster == 3 ~ "G2",
    dapi_area_cluster == 4 ~ "G01_large",
    dapi_area_cluster == 5 ~ "noise",
    TRUE ~ as.character(dapi_area_cluster)  # Handles any other unforeseen values
  ))
